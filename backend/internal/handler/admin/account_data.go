...
func (h *AccountHandler) importData(ctx context.Context, req DataImportRequest) (DataImportResult, error) {
	...
	for i := range dataPayload.Accounts {
		...
		if account.Extra != nil {
			if err := service.ValidateQuotaResetConfig(account.Extra); err != nil {
				result.AccountFailed++
				result.Errors = append(result.Errors, DataImportError{Kind: "account", Name: item.Name, Message: "invalid quota config: " + err.Error()})
				continue
			}
	...
	// Phase 2: 批量 INSERT（事务内：accounts + groups + 1 条 outbox）
	...
	if len(validAccounts) > 0 {
		...
		for _, acc := range validAccounts {
			if acc.Type == service.AccountTypeOAuth {
				if acc.Platform == service.PlatformAntigravity || acc.Platform == service.PlatformOpenAI {
	...
	// 异步设置 OAuth 隐私（Antigravity + OpenAI）
	if len(privacyAccounts) > 0 {
		adminSvc := h.adminService
		go func() {
			defer func() {
				if r := recover(); r != nil {
					slog.Error("import_oauth_privacy_panic", "recover", r)
				}
			}()
