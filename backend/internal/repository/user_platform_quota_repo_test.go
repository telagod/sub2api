...
func TestResetOrAccumulate(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if got := resetOrAccumulate(c.prevUsage, c.prevStart, c.currStart, c.cost); got != c.want {
				t.Errorf("resetOrAccumulate = %v, want %v", got, c.want)
			}
		})
	}
}
...
func TestMonthlyMaybeReset_NilStart(t *testing.T) {
	...
	usage, start := rollingMonthlyResetOrAccumulate(10.0, nil, 1.5, now)
	if usage != 1.5 {
...
func TestMonthlyMaybeReset_Expired(t *testing.T) {
	...
	usage, start := rollingMonthlyResetOrAccumulate(8.0, &windowStart, 2.0, now)
	if usage != 2.0 {
...
func TestMonthlyMaybeReset_CrossMonthBoundary(t *testing.T) {
	...
	usage, start := rollingMonthlyResetOrAccumulate(5.0, &windowStart, 1.0, now)
	if usage != 6.0 {
...
func TestMonthlyMaybeReset_Active(t *testing.T) {
	...
	usage, start := rollingMonthlyResetOrAccumulate(3.0, &windowStart, 0.5, now)
	if usage != 3.5 {
