...
import (
	...
	dbgroup "github.com/telagod/subme/ent/group"
	dbpredicate "github.com/telagod/subme/ent/predicate"
	dbproxy "github.com/telagod/subme/ent/proxy"
...
type accountRepository struct {
	...
	// cipher 负责 credentials 敏感子键的字段级加密（写路径）。可为 nil（降级明文）。
...
func accountEntityToService(m *dbent.Account) *service.Account {
	...
	return &service.Account{
		...
		Platform:                m.Platform,
		Type:                    m.Type,
		Credentials:             pkgCredentialCipher.DecryptMap(copyJSONMap(m.Credentials)),
