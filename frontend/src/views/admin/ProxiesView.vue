<template>
  <AppLayout>
      ...
      <template #table>
        ...
        >
          ...
          <template #cell-auth="{ row }">
            <div v-if="row.username || row.password" class="flex items-center gap-1.5">
              <div class="flex flex-col text-xs">
                <span v-if="row.username" class="text-foreground/85">{{ row.username }}</span>
                <span v-if="row.password" class="font-mono text-muted-foreground">
          ...
          <template #cell-expiry="{ row }">
            <span v-if="!row.expires_at" class="text-sm text-muted-foreground">{{ t('admin.proxies.neverExpires') }}</span>
            <div v-else class="flex flex-col text-xs">
              <span class="text-foreground/85">{{ formatDateTime(row.expires_at) }}</span>
              <span :class="expiryBadgeClass(row)">{{ expiryLabel(row) }}</span>
            </div>
          </template>

          <template #cell-created_at="{ row }">
            <span class="text-xs text-foreground/75">{{ formatDateTime(row.created_at) }}</span>
          </template>
        ...
        <div class="rounded-lg border border-border bg-card p-4">
          <div class="flex items-center justify-between gap-4">
            <div>
              ...
              <div class="mt-1 text-sm text-foreground/85">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 text-xs text-foreground/75">
            ...
            <tbody class="divide-y divide-border bg-white">
              <tr v-for="item in qualityReport.items" :key="item.target">
                ...
                <td class="px-3 py-2 text-foreground/75">
                  ...
                  <span v-if="item.cf_ray" class="ml-1 text-xs text-muted-foreground">(cf-ray: {{ item.cf_ray }})</span>
                </td>
              </tr>
            </tbody>
          </table>
    ...
    >
      <div v-if="accountsLoading" class="flex items-center justify-center py-8 text-sm text-muted-foreground">
        <Icon name="refresh" size="md" class="mr-2 animate-spin" />
        {{ t('common.loading') }}
      </div>
      <div v-else-if="proxyAccounts.length === 0" class="py-6 text-center text-sm text-muted-foreground">
        ...
        <table class="min-w-full divide-y divide-border text-sm">
          ...
          <tbody class="divide-y divide-border bg-white">
            <tr v-for="account in proxyAccounts" :key="account.id">
              ...
              <td class="px-4 py-2 text-foreground/75">
                {{ account.notes || '-' }}
              </td>
            </tr>
          </tbody>
