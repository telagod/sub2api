  ...
  <div class="rounded-lg bg-card p-6 shadow-metal border border-border">
    <div class="mb-4 flex items-start justify-between gap-4">
      <div>
        ...
        <p class="mt-1 text-xs text-muted-foreground">{{ t('admin.ops.alertRules.description') }}</p>
      </div>
    ...
    <div v-else-if="sortedRules.length === 0" class="rounded-md border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
      {{ t('admin.ops.alertRules.empty') }}
    </div>

    <div v-else class="max-h-[520px] overflow-hidden rounded-md border border-border">
      <div class="max-h-[520px] overflow-y-auto">
        <table class="min-w-full divide-y divide-border">
          ...
          <tbody class="divide-y divide-border bg-card">
            <tr v-for="row in sortedRules" :key="row.id" class="hover:bg-accent">
              <td class="px-4 py-3">
                ...
                <div v-if="row.updated_at" class="mt-1 text-[10px] text-muted-foreground">
                  {{ formatDateTime(row.updated_at) }}
                </div>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-foreground/85">
                <span class="font-mono">{{ row.metric_type }}</span>
                <span class="mx-1 text-muted-foreground">{{ row.operator }}</span>
                <span class="font-mono">{{ row.threshold }}</span>
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs font-bold text-foreground/85">
                {{ row.severity }}
              </td>
              <td class="whitespace-nowrap px-4 py-3 text-xs text-foreground/85">
    ...
    >
      <div class="space-y-4">
        <div v-if="!editorValidation.valid" class="rounded-md bg-red-500/10 border border-red-500/30 p-4 text-xs text-red-400">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <div v-if="selectedMetricDefinition" class="mt-1 space-y-0.5 text-xs text-muted-foreground">
          ...
          <div class="md:col-span-2">
            <label class="input-label">
              ...
              <span v-if="isGroupMetricSelected" class="ml-1 text-red-400">*</span>
            </label>
            <Select
          ...
          <div class="flex items-center justify-between rounded-md bg-muted px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-foreground/85">{{ t('admin.ops.alertRules.form.enabled') }}</span>
            <input v-model="draft!.enabled" type="checkbox" class="h-4 w-4 rounded border-border text-primary-600 focus:ring-ring" />
          </div>

          <div class="flex items-center justify-between rounded-md bg-muted px-4 py-3 md:col-span-2">
            <span class="text-xs font-bold text-foreground/85">{{ t('admin.ops.alertRules.form.notifyEmail') }}</span>
            <input v-model="draft!.notify_email" type="checkbox" class="h-4 w-4 rounded border-border text-primary-600 focus:ring-ring" />
          </div>
        </div>
      </div>
