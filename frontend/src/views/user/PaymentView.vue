      ...
      <template v-else>
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <Card v-if="validAmount > 0"><CardContent class="p-6">
              <div class="space-y-2 text-sm">
                ...
                <div v-if="feeRate > 0" class="flex justify-between border-t border-border pt-2">
                  <span class="font-medium text-foreground/85">{{ t('payment.actualPay') }}</span>
                  <span class="text-lg font-bold text-primary-200">{{ formatSelectedPaymentAmount(totalAmount) }}</span>
                </div>
            ...
            <button :class="'w-full rounded-md py-3 text-base font-medium ' + paymentButtonClass" :disabled="!canSubmit || submitting" @click="handleSubmitRecharge">
            ...
            <template v-if="selectedPlan">
              <Card><CardContent class="p-5">
                ...
                <div class="flex items-baseline gap-2">
                  <span v-if="selectedPlan.original_price" class="text-sm text-muted-foreground line-through">
                  ...
                  <span class="text-sm text-muted-foreground">/ {{ planValiditySuffix }}</span>
                </div>
                <!-- Description -->
                <p v-if="selectedPlan.description" class="mt-2 text-sm leading-relaxed text-muted-foreground">
              ...
              <Card v-if="feeRate > 0 && selectedPlan.price > 0"><CardContent class="p-6">
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-foreground">{{ formatSelectedPaymentAmount(selectedPlan.price) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-muted-foreground">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                    <span class="text-foreground">{{ formatSelectedPaymentAmount(subFeeAmount) }}</span>
                  </div>
              ...
              <button :class="'w-full rounded-md py-3 text-base font-medium ' + paymentButtonClass" :disabled="!canSubmitSubscription || submitting" @click="confirmSubscribe">
                <span v-if="submitting" class="flex items-center justify-center gap-2">
                  <span class="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent"></span>
                  {{ t('common.processing') }}
                </span>
                ...
                <div class="space-y-2">
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="flex flex-wrap gap-x-3 text-[11px] text-muted-foreground">
