      ...
      <template #table>
        ...
        >
          ...
          <template #cell-endpoint="{ row }">
            <span class="text-sm text-foreground/75 block max-w-[320px] whitespace-normal break-all">
          ...
          <template #cell-user_agent="{ row }">
            <span v-if="row.user_agent" class="text-sm text-foreground/75 block max-w-[320px] whitespace-normal break-all" :title="row.user_agent">{{ formatUserAgent(row.user_agent) }}</span>
            <span v-else class="text-sm text-muted-foreground">-</span>
          </template>
