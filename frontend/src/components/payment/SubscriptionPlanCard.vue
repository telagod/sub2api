<template>
  ...
  >
    ...
    <div class="flex flex-1 flex-col p-4">
      ...
      <div class="mb-3 grid grid-cols-2 gap-x-3 gap-y-1 rounded-md bg-muted px-3 py-2 text-xs">
        <div class="flex items-center justify-between">
          <span class="text-muted-foreground">{{ t('payment.planCard.rate') }}</span>
          <span class="font-medium text-foreground/85">{{ rateDisplay }}</span>
        </div>
        <div v-if="plan.daily_limit_usd != null" class="flex items-center justify-between">
          <span class="text-muted-foreground">{{ t('payment.planCard.dailyLimit') }}</span>
          <span class="font-medium text-foreground/85">${{ plan.daily_limit_usd }}</span>
        </div>
        <div v-if="plan.weekly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-muted-foreground">{{ t('payment.planCard.weeklyLimit') }}</span>
          <span class="font-medium text-foreground/85">${{ plan.weekly_limit_usd }}</span>
        </div>
        <div v-if="plan.monthly_limit_usd != null" class="flex items-center justify-between">
          <span class="text-muted-foreground">{{ t('payment.planCard.monthlyLimit') }}</span>
          <span class="font-medium text-foreground/85">${{ plan.monthly_limit_usd }}</span>
        </div>
        <div v-if="plan.daily_limit_usd == null && plan.weekly_limit_usd == null && plan.monthly_limit_usd == null" class="flex items-center justify-between">
          <span class="text-muted-foreground">{{ t('payment.planCard.quota') }}</span>
          <span class="font-medium text-foreground/85">{{ t('payment.planCard.unlimited') }}</span>
        </div>
      ...
      <div v-if="plan.features.length > 0" class="mb-3 space-y-1">
        <div v-for="feature in plan.features" :key="feature" class="flex items-start gap-1.5">
          ...
          <span class="text-xs text-foreground/85">{{ feature }}</span>
        </div>
      </div>
