<template>
  ...
  >
    <div class="space-y-4">
      ...
      <div v-else-if="!isActive" class="rounded-md border border-border p-4 text-sm text-muted-foreground">
        {{ t('admin.accounts.tempUnschedulable.notActive') }}
      </div>

      <div v-else class="space-y-4">
        <div class="rounded-md border border-emerald-500/30 bg-emerald-500/10 p-3 text-sm text-emerald-400">
        ...
        <div class="rounded-md border border-border p-3">
          ...
          <div class="mt-2 rounded bg-muted p-2 text-xs text-foreground/85">
            {{ state?.error_message || '-' }}
          </div>
        </div>
      </div>
