<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="isRateLimited" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-muted-foreground">{{ rateLimitResumeText }}</span>
    </div>
    ...
    <div v-else-if="isOverloaded" class="flex flex-col items-center gap-1">
      ...
      <span class="text-[11px] text-muted-foreground">{{ overloadCountdown }}</span>
    </div>
    ...
    <div v-if="hasError && account.error_message" class="group/error relative">
      <svg
        class="h-4 w-4 cursor-help text-red-400 transition-colors hover:text-red-300"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        stroke-width="2"
      ...
      >
        ...
        <!-- 上方小三角 -->
        <div
          class="absolute bottom-full left-3 border-[6px] border-transparent border-b-[#2a2d31]"
        ></div>
      </div>
    </div>
    ...
    <div v-if="isRateLimited" class="group relative">
      <span
        class="inline-flex items-center gap-1 rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-xs font-medium text-amber-400"
      >
      ...
      >
        ...
        <div
          class="absolute left-1/2 top-full -translate-x-1/2 border-4 border-transparent border-t-[#2a2d31]"
        ></div>
      </div>
    </div>
    ...
    >
      <div v-for="item in activeModelStatuses" :key="`${item.kind}-${item.model}`" class="group relative mb-1 break-inside-avoid">
        ...
        <span
          ...
          class="inline-flex items-center gap-1 rounded border border-red-500/30 bg-red-500/10 px-1.5 py-0.5 text-xs font-medium text-red-400"
        >
          <Icon name="exclamationTriangle" size="xs" :stroke-width="2" />
