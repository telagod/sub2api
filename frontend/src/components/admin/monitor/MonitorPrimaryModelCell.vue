<template>
  <div class="flex items-center gap-2">
    <span class="text-sm text-foreground">{{ row.primary_model }}</span>
    <HelpTooltip>
      <template #trigger>
        <span
      ...
      <div class="space-y-2">
        ...
        <div v-if="(row.extra_models?.length ?? 0) === 0" class="text-[11px] text-muted-foreground">
          ...
          <table class="w-full text-left text-[11px]">
            ...
            <tbody>
              <tr v-for="m in (row.extra_models_status || [])" :key="m.model">
                <td class="py-0.5 pr-2 text-foreground">{{ m.model }}</td>
                ...
                <td class="py-0.5 text-foreground">{{ formatLatency(m.latency_ms) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
