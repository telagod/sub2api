<template>
  ...
  >
    <div class="space-y-3">
      ...
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-muted-foreground">
        ...
        <table class="w-full text-sm">
          ...
          <tbody class="divide-y divide-border">
            <tr
              ...
              @click="$emit('pick', k)"
            >
              <td class="px-3 py-2 font-medium text-foreground">{{ k.name }}</td>
              <td class="px-3 py-2 font-mono text-xs text-muted-foreground">{{ maskApiKey(k.key) }}</td>
              <td class="px-3 py-2">
                <GroupBadge
                  v-if="k.group"
                  :name="k.group.name"
