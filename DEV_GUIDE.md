...
Phase 1: 重写独立模块（高价值，改进集成）           预计 3-5 天
    ├─ Channel Monitor → 统一 observability 层
    ├─ Content Moderation → 可插拔 middleware chain
    ├─ User Platform Quota → 合并进现有 billing/usage
    ├─ DingTalk Auth → 统一 OAuth provider interface
    ├─ Affiliate → 独立推荐/返佣模块
    └─ 新 Ent Schema + go generate
